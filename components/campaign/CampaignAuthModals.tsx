...
export function CampaignAuthModals() {
  const { openModal, openSignIn, openSignUp, close } = useCampaignModals();

  return (
    <>
      <CampaignModalShell open={openModal === "signin"} onClose={close} title="Sign In">
        <SignInModal onSwitchToSignUp={openSignUp} onSuccess={close} />
      </CampaignModalShell>

      <CampaignModalShell open={openModal === "signup"} onClose={close} title="Join the Campaign">
        <SignUpModal onSwitchToSignIn={openSignIn} />
      </CampaignModalShell>
    </>
  );
}
